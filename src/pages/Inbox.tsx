  ...
  return (
    <div className="page-content fade-in">
      ...
      {pending.length === 0 ? (
        <div className="pip-panel" style={{ marginBottom: 12 }}>
          <div className="pip-panel-header">
            <span className="pip-panel-title">INBOX ZERO</span>
            <span className="pip-panel-meta">CLEAR</span>
          </div>
          <div className="pip-panel-body">
            <div className="pip-empty" style={{ padding: '18px 0' }}>
              <div className="pip-empty-title">NO LOOSE ENDS</div>
        ...
        <div className="pip-panel" style={{ marginBottom: 12 }}>
          <div className="pip-panel-header">
            <span className="pip-panel-title">READY TO TRIAGE</span>
            <span className="pip-panel-meta">{pending.length} ITEMS</span>
          </div>
          <div className="pip-panel-body" style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
            {pending.map((item) => {
              ...
              return (
                <div key={item.id} data-domain={domainId} style={{ ...getDomainThemeStyle(domainId, domains), border: '1px solid var(--pip-border)', background: 'var(--pip-faint)', padding: '10px 12px' }}>
                  <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 12, marginBottom: 6 }}>
                    <div style={{ display: 'flex', alignItems: 'center', gap: 8, flexWrap: 'wrap' }}>
                      <span style={{ fontFamily: 'var(--font-display)', fontSize: 15, color: 'var(--domain-primary)', letterSpacing: 1 }}>
                        {item.domain_id ? getDomainLabel(item.domain_id, domains).toUpperCase() : 'UNSORTED'}
                      </span>
                      <span style={{ fontSize: 10, color: 'var(--pip-muted)', letterSpacing: 1 }}>
                  ...
                  <div style={{ fontSize: 15, color: 'var(--pip-bright)', marginBottom: 10, whiteSpace: 'pre-wrap' }}>
                    {item.content}
                  </div>
      ...
      <div className="pip-panel">
        <div className="pip-panel-header">
          <span className="pip-panel-title">SOMEDAY / MAYBE</span>
          <span className="pip-panel-meta">{someday.length} PARKED</span>
        </div>
        <div className="pip-panel-body" style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
          {someday.length === 0 ? (
            ...
            someday.map((item) => (
              <div key={item.id} data-domain={item.domain_id ?? undefined} style={{ border: '1px solid var(--pip-border)', padding: '8px 10px', background: 'var(--pip-faint)' }}>
                <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 10 }}>
                  <div style={{ fontSize: 13, color: 'var(--pip)' }}>{item.content}</div>
                  <div style={{ fontSize: 10, color: 'var(--pip-muted)', letterSpacing: 1 }}>{SOURCE_LABELS[item.source_label]}</div>
                </div>
