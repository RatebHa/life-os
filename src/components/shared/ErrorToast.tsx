    ...
    <div className="fixed bottom-4 left-4 z-[300] space-y-2 pointer-events-none">
      {errors.map((err) => (
        ...
        >
          ...
          <span className="text-xs text-[var(--color-text-primary)] flex-1 leading-snug">{err.message}</span>
          <button
            onClick={() => dismissError(err.id)}
            className="flex-shrink-0 text-[var(--color-text-dim)] hover:text-[var(--color-text-primary)] transition-colors"
          >
            <X size={12} />
          </button>
        </div>
